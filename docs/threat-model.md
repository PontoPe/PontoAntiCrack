...
| R4 | Event schema drift → detection stops matching, silently | High | High | Fixture-based unit tests in CI; coverage guard that also requires a negative assertion; dead-letter alarm; `make patterns` replays every fixture through EventBridge itself; fifteen of seventeen fixtures are captured events. | implemented | **This threat was realised and caught by detonation, not by any of the above.** `AuthorizeSecurityGroupIngress` has two CloudTrail encodings and every fixture had been produced by the AWS CLI, so the pattern only knew one; an attacker using the legacy top-level parameters was invisible while every gate stayed green. Both encodings are covered now. The residual is the same shape: a *third* encoding, or a service that changes one, is still only discoverable by detonating. |
...
| R9 | Remediation re-triggers its own detection (invocation loop) | Med | Med | The runtime drops events whose **role name** starts with `pac-`, before planning. Tested, including the bypass below. | implemented | **The first version tested the whole ARN, which contains the caller-chosen session name.** Any attacker passing `--role-session-name pac-anything` was classified as this system's own automation and skipped, with a confident `SKIPPED` in the audit trail. Found by detonation: Stratus ran under `pac-terraform` and the detection dropped its own attack. Residual: still a naming convention, but on the role, which is fixed at creation rather than chosen at assume time. |
